...
// expansion. Built-ins always win, so an alias can never shadow `jobs`, `config`, etc.
...
func init() {
	metaRegistrars = append(metaRegistrars, func(d *deps) *cobra.Command {
		...
		setCmd := &cobra.Command{
			...
			Example: `  remoteok alias set go "jobs list --tag golang"
  remoteok alias set rich "jobs list --min-salary 150000"`,
