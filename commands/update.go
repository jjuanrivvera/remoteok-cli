...
func init() {
	metaRegistrars = append(metaRegistrars, func(_ *deps) *cobra.Command {
		cmd := &cobra.Command{
			Use:   "update",
			Short: "Update remoteok to the latest GitHub release",
			Long: `Download the latest remoteok release, verify it against checksums.txt, and replace
the running binary in place. Use 'remoteok update check' to see what's available
without installing.`,
			Example: `  remoteok update
...
func newUpdateCheckCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			default:
				fmt.Fprintln(out, "A newer version is available. Run `remoteok update` to install it.")
			}
			return nil
		},
	}
