...
var localGroups = []string{"config", "alias", "init", "doctor", "completion", "version", "update", "agent", "mcp", "help"}
...
func init() {
	metaRegistrars = append(metaRegistrars, func(_ *deps) *cobra.Command {
		agentCmd := &cobra.Command{
			...
			Long:  "Generate safety configuration for AI agents that drive remoteok.",
		}
		...
		guard := &cobra.Command{
			...
			Short: "Generate agent-safety config for AI agents that drive remoteok",
...
accepted: a line that merely QUOTES a blocked command is denied.`,
			Example: `  remoteok agent guard --host claude-code
...
func writeClaudeCodeFiles(cmd *cobra.Command, cls classification) error {
	...
	files := map[string]string{
		".claude/hooks/remoteok-guard.sh":      hook,
		".claude/remoteok-guard.settings.json": settings,
	}
	...
	fmt.Fprintln(cmd.ErrOrStderr(), "merge .claude/remoteok-guard.settings.json into .claude/settings.json to activate the hook")
	return nil
}
...
func bashPattern(path string) string { return "Bash(remoteok " + path + ":*)" }

// mcpToolPattern is the MCP tool name a host gates: remoteok_<group>_<verb> under the remoteok
...
func mcpToolPattern(path string) string {
	return "mcp__remoteok__remoteok_" + strings.ReplaceAll(path, " ", "_")
}
