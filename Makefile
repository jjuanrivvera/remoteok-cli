# Makefile — remoteok (Remote OK jobs CLI), built to the cliwright standard.
# `make verify` is the deterministic acceptance gate; `make accept` adds the LLM judge.
BINARY      := remoteok
MODULE      := github.com/jjuanrivvera/remoteok-cli
...
COVERAGE_MIN ?= 80
# spec-completeness: min % of the enumerated Remote OK API the manifest must wrap
API_COVERAGE_MIN ?= 90
...
# verify == the DETERMINISTIC gate (build/test/lint/spec/completeness/coverage/DoD).
